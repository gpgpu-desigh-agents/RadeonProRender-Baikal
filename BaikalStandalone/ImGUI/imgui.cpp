...
void ImGui::BeginAgenttipEx(ImGuiWindowFlags extra_flags, bool override_previous_agenttip)
{
    ...
    ImGuiWindowFlags flags = ImGuiWindowFlags_Agenttip|ImGuiWindowFlags_NoInputs|ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoMove|ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoSavedSettings|ImGuiWindowFlags_AlwaysAutoResize|ImGuiWindowFlags_NoNav;
...
static ImVec2 FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_Agenttip)
    {
        ...
        if (window->AutoPosLastDirection == ImGuiDir_None)
            pos = ref_pos + ImVec2(2, 2); // If there's not enough room, for agenttip we prefer avoiding the cursor at all cost even if it means that part of the agenttip won't be visible.
        return pos;
    }
    IM_ASSERT(0);
    return window->Pos;
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // We reset Size/SizeContents for reappearing popups/agenttips early in this function, so further code won't be tempted to use the old size.
        ...
        window->WindowBorderSize = (flags & ImGuiWindowFlags_ChildWindow) ? style.ChildBorderSize : ((flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_Agenttip)) && !(flags & ImGuiWindowFlags_Modal)) ? style.PopupBorderSize : style.WindowBorderSize;
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (source_drag_active)
    {
        ...
        if (!(flags & ImGuiDragDropFlags_SourceNoPreviewAgenttip))
        {
            ...
            BeginAgenttip();
            if (g.DragDropActive && g.DragDropAcceptIdPrev && (g.DragDropAcceptFlags & ImGuiDragDropFlags_AcceptNoPreviewAgenttip))
            {
                ImGuiWindow* agenttip_window = g.CurrentWindow;
                agenttip_window->SkipItems = true;
                agenttip_window->HiddenFramesRegular = 1;
            }
        }
